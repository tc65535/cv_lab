...
# need to go back to top directory to read and save data
root = "../"
...
def write_and_show(filename, img):
    success = cv2.imwrite(root + filename, img)
...
def read_video_frames(video_name):
    ...
    cap = cv2.VideoCapture(root + video_name)
...
def write_frames_to_video(video_name, frames, fps):
    ...
    out = cv2.VideoWriter(root + video_name, cv2.VideoWriter_fourcc('a', 'v', 'c', '1'), fps, (W, H))
